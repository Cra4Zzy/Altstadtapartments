...
# One-time migration to directory-index URLs so every public page can use a clean path.
ROOT_ROUTES = [
...
for slug in ROOT_ROUTES:
    ...
    source.write_text(redirect_stub(f'{slug}/', f'{DOMAIN}/{slug}/', 'de'), encoding='utf-8')
...
for slug in EN_ROUTES:
    ...
    destination.parent.mkdir(parents=True, exist_ok=True)
    destination.write_text(clean, encoding='utf-8')
    source.write_text(redirect_stub(f'{slug}/', f'{DOMAIN}/en/{slug}/', 'en'), encoding='utf-8')
